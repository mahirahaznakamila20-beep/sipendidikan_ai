...
(async () => {
  ...
  // create two schools
  const s1 = await req('/api/admin/schools', 'POST', { school_name: 'MI 1 Rajadesa', npsn: '11111111' }, adminToken);
  console.log('Create school 1:', s1.status, s1.body && s1.body.id ? 'id='+s1.body.id : s1.body);
  const s2 = await req('/api/admin/schools', 'POST', { school_name: 'SD Negeri 1 Jakarta', npsn: '66666544' }, adminToken);
  console.log('Create school 2:', s2.status, s2.body && s2.body.id ? 'id='+s2.body.id : s2.body);

  // register Agus (school npsn 11111111)
  const regAgus = await req('/api/auth/register', 'POST', { username: 'agus', email: 'agus@example.com', password: 'Pass1234!', school_npsn: '11111111' });
  console.log('Register Agus:', regAgus.status);
  // register Asep
  const regAsep = await req('/api/auth/register', 'POST', { username: 'asep', email: 'asep@example.com', password: 'Pass1234!', school_npsn: '66666544' });
  console.log('Register Asep:', regAsep.status);

  // login Agus and Asep
  const loginAgus = await req('/api/auth/login', 'POST', { identifier: 'agus@example.com', password: 'Pass1234!' });
  const tokenAgus = loginAgus.body?.data?.accessToken || loginAgus.body?.accessToken; console.log('Login Agus:', loginAgus.status);
  const loginAsep = await req('/api/auth/login', 'POST', { identifier: 'asep@example.com', password: 'Pass1234!' });
  ...
  const createStudent = await req('/api/teacher/students', 'POST', { student_name: 'siswa_agus', nisn: '1234567890', password: 'siswaPW' }, tokenAgus);
  ...
  const isolated = (attemptUpdate.status === 404 || attemptUpdate.status === 403) && aseps.every(s => s.nisn !== '1234567890');
